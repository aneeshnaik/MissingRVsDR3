...
def posterior(q, alpha, beta, L, w, wzp, sig_w):
    ...
    p_li[np.all(p_li <= 1e-200, axis=-1)] = 1
...
if __name__ == "__main__":
    ...
    for i, df in tqdm(enumerate(df_iter), total=N_chunks):
        ...
        prior_data = pd.read_csv(ddir + '/External/BJ_distances_prior.csv')
        ...
        # get samples
        # if 3000 too few points for CDF integration try 15000 then 75000
        try:
            ...
            try:
                ...
                q_samples = sample(N=N_samples, N_pts=75000, **sample_args)
