...
def create_plot_data(dfile, N_bins, X_min, X_max, Y_min, Y_max):
    ...
    # construct maps
    print("5D map:")
    mu5 = calc_vlos_map(X5, Y5, V5, N_bins, X_min, X_max, Y_min, Y_max)
    print(">>>Done.\n")
    print("6D map:")
    mu6 = calc_vlos_map(X6, Y6, V6, N_bins, X_min, X_max, Y_min, Y_max)
    print(">>>Done.\n")
...
if __name__ == "__main__":
    ...
    # plot settings
    imargs = dict(
        origin='lower', vmax=110, vmin=-110, extent=[X_max, X_min, Y_max, Y_min],
    ...
    for R in np.arange(2, 16, 2):
        phi = np.linspace(np.pi / 2, 3 * np.pi / 2, 500)
        X = R * np.cos(phi)
        Y = R * np.sin(phi)
    ...
    # radial grid lines
    for phi in np.linspace(np.pi / 2, 3 * np.pi / 2, 9):
