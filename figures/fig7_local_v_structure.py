...
# -*- coding: utf-8 -*-
"""
2D histograms of local velocity (U/V) field. Moving groups etc.
...
if __name__ == "__main__":
    ...
    # load plot data
    ddir = get_datadir()
    dfile = ddir + 'figures/fig7_local_v_structure_data.npz'
    if not exists(dfile):
    ...
    fig.suptitle("6D test set: local velocity distribution", x=left + 0.5 * (right - left))
    ...
    # ticks, labels, etc.
    ax1.set_xlabel(r'$U$ [km/s]', usetex=True)
    ax0.set_ylabel(r'$V$ [km/s]', usetex=True)
    ax1.set_ylabel(r'$V$ [km/s]', usetex=True)
    for ax in [ax0, ax1]:
        ax.tick_params(right=True, top=True, direction='inout')
    ax0.tick_params(labelbottom=False)
    ax0.text(0.05, 0.95, r"Measured radial velocities", ha='left', va='top', transform=ax0.transAxes)
    ax1.text(0.05, 0.95, r"Predicted radial velocities", ha='left', va='top', transform=ax1.transAxes)
    cax.set_ylabel("Density [arbitrary units]")

    # save
    fig.savefig("fig7_local_v_structure.pdf", dpi=800)
